...
human_mouse_sig_set <- human_sig_mapped %>%
...
################################################################################
compare_one_file <- function(mouse_file, out_dir, human_sig_mapped_tbl, extra_sets = list()) {
  ...
  overlap_schwann <- if (!is.null(extra_sets$Schwann_noMito)) length(intersect(m_genes, extra_sets$Schwann_noMito)) else NA_integer_
  overlap_jci     <- if (!is.null(extra_sets$JCI_bulk)) length(intersect(m_genes, extra_sets$JCI_bulk)) else NA_integer_
  ...
  summary_row <- tibble::tibble(
    ...
    n_overlap       = nrow(both),
    n_overlap_schwann = overlap_schwann,
    n_overlap_jci     = overlap_jci,
  ...
  # 2-way Venn (Human vs Mouse)
  venn_two_base <- file.path(out_dir, paste0(base, "_Venn"))
  plot_and_save_venn(
    list(Human_to_Mouse = h_genes, Mouse = m_genes),
    title = paste0("Overlap: ", base),
    file_base = venn_two_base
  )
  
  # 4-way Venn (Mouse vs Human vs Schwann vs JCI bulk)
  venn_sets <- list(
    Mouse                = m_genes,
    HumanSural_to_Mouse  = h_genes,
    Schwann_noMito       = extra_sets$Schwann_noMito,
    JCI_bulk             = extra_sets$JCI_bulk
  )
  venn_sets <- Filter(function(x) !is.null(x) && length(x) > 0, venn_sets)
  if (length(venn_sets) >= 3) {
    venn_four_base <- file.path(out_dir, paste0(base, "_Venn4way"))
    plot_and_save_venn(
      venn_sets,
      title = paste0("Mouse/Human/Schwann/JCI: ", base),
      file_base = venn_four_base
    )
  }
...
################################################################################
compare_folder <- function(input_dir, output_dir, human_sig_mapped_tbl, extra_sets) {
  ...
  all_summaries <- lapply(files, function(f) {
    tryCatch(
      compare_one_file(f, per_file_out, human_sig_mapped_tbl, extra_sets = extra_sets),
      error = function(e) {
        ...
        tibble::tibble(
          ...
          n_overlap = NA_integer_, n_overlap_schwann = NA_integer_, n_overlap_jci = NA_integer_, jaccard = NA_real_,
  ...
  if (length(sel_files) > 0) {
    ...
    sel_summaries <- lapply(sel_files, function(f) {
      tryCatch(
        compare_one_file(f, per_file_sel_out, human_sig_mapped_tbl, extra_sets = extra_sets),
        error = function(e) {
          warning("Failed on ", basename(f), ": ", conditionMessage(e))
          parts <- strsplit(safe_base(tools::file_path_sans_ext(basename(f))), "_", fixed = TRUE)[[1]]
          comp_group <- if (length(parts) >= 1) parts[1] else NA_character_
...
invisible(mapply(function(in_dir, out_dir) {
  compare_folder(in_dir, out_dir, human_sig_mapped, extra_sets = extra_mouse_sets)
